...
class User(db.Model):
    ...
    last_login = db.Column(db.DateTime(timezone=True),default=func.now(), nullable=True)
...
class Assignment(db.Model):
    ...
    course_id = db.Column(db.Integer, db.ForeignKey('course.id', ondelete="CASCADE"), nullable=False)
    created_at = db.Column(db.DateTime(timezone=True), default=func.now())
    assignment_type = db.Column(db.String(50), nullable=False, default="Assignment")
...
class UserAssignmentScore(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey('user.id', ondelete="CASCADE"), nullable=False)
    assignment_id = db.Column(db.Integer, db.ForeignKey('assignment.id', ondelete="CASCADE"), nullable=False)
    score = db.Column(db.Float, nullable=False, default=0.0)
    graded_at = db.Column(db.DateTime(timezone=True), default=func.now())

    user = db.relationship('User', backref='assignment_scores', lazy=True)
    assignment = db.relationship('Assignment', backref='assignment_scores', lazy=True)
