...
def client():
    ...
    def run_code():
        # Mock the RunCode resource logic here for testing
        # This should match the logic in your actual resource
        data = request.get_json()
        # Add the same logic from your resource class for testing
        return jsonify({"message": "Code run successfully"})

    return app.test_client()

def test_run_code(client: FlaskClient):
    response = client.post('/run_code', json={
        "code": "def add(a, b): return a + b",
        "test_cases": [{"input": "1, 2", "expected_output": "3"}]
    })
    assert response.status_code == 200
    assert b"Code run successfully" in response.data

def test_submit_code(client: FlaskClient):
...
def test_update_score(client: FlaskClient):
    # Mock a valid JWT token
    headers = {
        'Authorization': 'Bearer your-jwt-token-here'
    }

    response = client.post('/update_score', json={
        "assignment_id": 1,
        "score": 10
    }, headers=headers)

    assert response.status_code == 200
    assert b"Score updated successfully" in response.data
